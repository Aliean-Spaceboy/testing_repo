...
// Extract advMessages and advSpeechRec
code = code.replace(/let advMessages = \[\];\n/, '');
code = code.replace(/let advSpeechRec = null;\n/, '');
let header = `
let advMessages = [];
let advSpeechRec = null;
`;

const modContent = `// js/adventure.js
` + header + '\n\n' + extractedCode.join('\n\n');

fs.writeFileSync('js/adventure.js', modContent.trim());

// Update app.js
let appJs = fs.readFileSync('js/app.js', 'utf8');

const exportsToAdd = funcsToExtract;
appJs = appJs.replace(`import '../script.js';`, `import * as adventure from './adventure.js';
window.adventure = adventure;
${exportsToAdd.map(f => `window.${f} = adventure.${f};`).join('\n')}
